...
def preprocess_masks(df, experiment_data, experiment_model, unmasker):
    ...
    for i in range(len(df)):
      mask = df['masked_sentences'][i].replace('Ä', '').replace('Ġ', '')
      if mask.count('[MASK]') > 1 or mask.count('[MASK]') == 0:
        valid_examples -= 1
        continue
      if 're' in experiment_data:
        gold_label = df['obj'][i].replace('Ä', '').replace('Ġ', '')
      else:
        gold_label = df['obj_label'][i].replace('Ä', '').replace('Ġ', '')
      ...
      top_pred = preds[0].replace('Ä', '').replace('Ġ', '')
      if 'Roberta' in experiment_model:
...
def run_experiments(tokenizer, model, unmasker, experiment_model, extractor, dataset=None):
    retrieve_data()
    for experiment_data in ['squad', 're-date-birth', 're-place-birth', 're-date-death']:
        if dataset and dataset != experiment_data:
            continue
        
        df = generate_df(experiment_data)
        good_preds_ids, predicted_sentences, ground_truth_sentences, \
                        valid_examples = preprocess_masks(df, experiment_data, experiment_model, unmasker)
        
        prefix = experiment_data + '_' + experiment_model + '_' + extractor
        # language model predictions
        kg_df, label_dict = generate_kg(predicted_sentences, extractor)
        kg_df.to_csv(prefix + '_kg_df.csv')
        
        plot_kg(kg_df, label_dict, 'skyblue', 'red', \
                prefix + '_kg.jpg')
        
        # ground truth predictions
        g_kg_df, g_label_dict = generate_kg(ground_truth_sentences, extractor)
        g_kg_df.to_csv(prefix + '_g_kg_df.csv')
        ...
        intersection_df.to_csv(prefix + '_intersection.csv')
        plot_kg(intersection_df, dict(intersection_labels), 'green', 'green', \
                prefix + '_intersection.jpg')
